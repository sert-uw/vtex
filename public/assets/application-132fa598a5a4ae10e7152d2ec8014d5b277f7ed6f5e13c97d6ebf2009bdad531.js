...
(function() {
  $(function() {
    $('#file_text_area').on('keyup', function(e) {
      if (e.keyCode < 37 || 40 < e.keyCode) {
        return $('#update_button').trigger('click');
      }
    }).autosize().hlightTextarea({
      words: [
        {
          color: '#EEEEEE',
          words: ['%.*\n']
          ...
          color: '#E6E6FA',
          words: ['begin{document}']
        }, {
          color: '#FFFFE0',
          words: ['\\\\', 'begin', 'documentclass', 'usepackage', 'renewcommand', 'bibname', 'vspace']
        }
      ]
    });
    $("a[data-remote]").on('ajax:complete', function(event, ajax, status) {
      var file, method, response;
      response = $.parseJSON(ajax.responseText);
      method = response.data.method;
      file = response.data.file;
      return $(this).closest('#' + file).remove();
    });
    $('#update').on('ajax:complete', function(event, ajax, status) {
      var file, response;
      response = $.parseJSON(ajax.responseText);
      return file = response.data.file;
    });
    $('#pdf_update').on('ajax:complete', function(event, ajax, status) {
      var path, response;
      response = $.parseJSON(ajax.responseText);
    ...
    $('#pdf_update_under').on('click', function() {
      return $('#pdf_update').trigger('click');
    });
    return $('#create').on('ajax:complete', function(event, ajax, status) {
      var html, response, tag_id;
      response = $.parseJSON(ajax.responseText);
      ...
      return $('#' + tag_id).on('ajax:complete', function(event, ajax, status) {
        var file, method;
        response = $.parseJSON(ajax.responseText);
        method = response.data.method;
        file = response.data.file;
